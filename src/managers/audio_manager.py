...
#         # Start processing loop in a separate process
#         self.processing_process = multiprocessing.Process(target=self.async_processing_loop)
#         self.processing_process.start()

#     async def async_processing_loop(self):
#         while self.is_recording:
#             await asyncio.sleep(10)  # Adjust the frequency of checks
#             # Async combine and transcribe if enough new snippets are available
#             if len(self.new_snippets) >= 1:
#                 await self.combine_and_transcribe_async()

#     async def combine_and_transcribe_async(self):
#         # Use ThreadPoolExecutor to run blocking IO tasks in separate threads
#         with ThreadPoolExecutor(max_workers=2) as executor:
#             loop = asyncio.get_event_loop()
            
#             # Async combining audio files
#             combine_future = loop.run_in_executor(executor, self._combine_audio_files)
#             await combine_future  # Wait for combining to complete

#             # Async transcribing combined audio
#             transcribe_future = loop.run_in_executor(executor, self.get_transcript, self.combined_audio_path)
#             transcript = await transcribe_future  # Wait for transcribing to complete
            
#             # Handle the transcript (e.g., update UI, save to file, etc.)
#             if transcript:
...
class AudioManager:
    ...
    def start_recording(self):
        ...
        self.processing_thread = threading.Thread(target=self.async_processing_loop, daemon=True)
    ...
    #         # Call the check_and_process_updates method
    #         self.check_and_process_updates()
